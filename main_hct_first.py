...
def get_args_parser():
    parser = argparse.ArgumentParser('HCTransformers-First', add_help=False)
...
'''
mini:   mean = [x/255.0 for x in [120.39586422,  115.59361427, 104.54012653]]
...
cifar:  mean = [x/255.0 for x in [129.37731888,  124.10583864, 112.47758569]]
        std = [x/255.0 for x in [68.20947949,  65.43124043,  70.45866994]]      

FC100:  mean = [x/255.0 for x in [129.37731888,  124.10583864, 112.47758569]]
        std = [x/255.0 for x in [68.20947949,  65.43124043,  70.45866994]]  
...
def train_dino(args):
    ...
    student = utils.MultiCropWrapper_hct(student, DINOHead(
        embed_dim,
        args.out_dim,
        use_bn=args.use_bn_in_head,
        norm_last_layer=args.norm_last_layer,
    ),'student')
    teacher = utils.MultiCropWrapper_hct(
        teacher,
    ...
    lr_schedule = utils.cosine_scheduler(
        ...
        args.lr * (16 * 8) / 32.,  # linear scaling rule
        args.min_lr,
    ...
    utils.restart_from_checkpoint(
        ...
        dino_loss=dino_loss,
        surrogate_loss1=surrogate_loss1,
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        train_stats = train_one_epoch(student, teacher, teacher_without_ddp, dino_loss,surrogate_loss1,surrogate_loss2,
        ...
        save_dict = {
            ...
            'surrogate_loss1':surrogate_loss1.state_dict(),
            'surrogate_loss2':surrogate_loss2.state_dict(),
        }
...
def train_one_epoch(student, teacher, teacher_without_ddp, dino_loss,surrogate_loss1, surrogate_loss2, data_loader,
    ...
    for it, (images, labels) in enumerate(metric_logger.log_every(data_loader, 10, header)):
        ...
        with torch.cuda.amp.autocast(fp16_scaler is not None):
            ...
            student_output, student_tokens,  = student(images)
            dinoloss = dino_loss(student_output, teacher_output, epoch)
            surrogateloss1 = surrogate_loss1(student_output, labels) # alpha
            if epoch <= 250:
                surrogateloss2 = surrogate_loss2(student_tokens, labels) * 0.1 # beta
                loss = dinoloss + surrogateloss1 + surrogateloss2
            else:
                loss = dinoloss + surrogateloss1
        ...
        metric_logger.update(clsloss=surrogateloss1.item())
        if epoch <= 250:
...
class SurrogateLoss(nn.Module):
    ...
    """
    def __init__(self, ncrops, num_classes=64, feat_dim=8192, use_gpu=True,surrogate_momentum=0.999999):
    ...
    def forward(self, x, labels):
        ...
        for i in range(2):
            ...
            kl = F.kl_div(F.log_softmax(x[i],dim=-1),F.softmax(surrogate,dim=-1),reduction='batchmean')
            loss = torch.clamp(kl, min=1e-5, max=1e+5).mean(dim=-1)
            total_loss += loss
            self.update_surrogate(labels,x[i])
        return total_loss/2 * 16 # 16; base batch size
